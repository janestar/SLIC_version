...
/**
 * This class estimates the parameters of 3D planes.
 * A 3D plane is represented as: (*) dot(n,p-a)=0 
...
class PlaneParamEstimator : public ParameterEsitmator<Node,double> 
{
public:
	 ...
	 */
	virtual void estimate(std::vector<Node *> &data, std::vector<double> &parameters);
	 ...
	 *                   vector is empty (size = 0).
	 */
	virtual void leastSquaresEstimate(std::vector<Node *> &data, std::vector<double> &parameters);
	 ...
	 */
	virtual bool agree(std::vector<double> &parameters, Node &data);

...
void Holefilling::Normalized_Convolution(const int& r,const int& c,vector<int>& vec){
	...
	if(r>0&&r<height&&c>0&&c<width){
		int mark=c+r*width;
	if(img.at<uchar>(r,c)==0&&!vec[mark]){
			uchar data_left =0;
			uchar data_right =0;
			uchar data_top =0;
			uchar data_bottom = 0;
			uchar data_left_top=0;
			uchar data_left_bottom=0;
			uchar data_right_top=0;
			uchar data_right_bottom=0;
			...
			double sigma=10; 
			if(c-1>=0)data_left = img.at<uchar>(r,c-1);
			if(c+1<width)data_right = img.at<uchar>(r,c+1);
			if(r-1>=0)data_top = img.at<uchar>(r-1,c);
			if(r+1<height)data_bottom = img.at<uchar>(r+1,c);
			if(c-1>=0&&r-1>=0)data_left_top = img.at<uchar>(r-1,c-1);
			if(c+1<width&&r-1>=0)data_right_top = img.at<uchar>(r-1,c+1);
			if(c-1>=0&&r+1<height)data_left_bottom = img.at<uchar>(r+1,c-1);
			if(r+1<height&&c+1<width)data_right_bottom = img.at<uchar>(r+1,c+1);
			...
			||data_left_top||data_right_top||data_left_bottom||data_right_bottom){
			  ...
			  img.at<uchar>(r,c)=saturate_cast<uchar>(depth);
			   vec[mark]=1;
			   Node n;
			   n.setX(r);
			   n.setY(c);
